  ...
  const onInform = () => {
    ...
    if (confirmInform) {
      ...
      if (!logined) { // ตอนลงทะเบียน
        ...
        dispatch(setUserConsentAction.request({ result: true }));
        setLoading(false);
      }
    } else {
      setWarning(true);
  ...
  return (
    <Modal
            ...
            {logined && (
              <Row style={{ marginTop: '10px' }}>
                <Col style={{ fontSize: (isMobile ? ".9em" : "1em"), fontWeight: 'bold', lineHeight: '1.3em', color: (informed ? '#3bb300' : 'red') }}>
                  <Trans>{informed ? "consent.result.yes" : "consent.result.no"}</Trans>
                </Col>
              </Row>
            )}
          </Col>
      ...
      footer={
        informed === true ?
          [
            <Button key="cancel" size={isMobile ? "small" : "middle"} onClick={() => setShowModalConsent(false)}> <Trans>consent.btn.cancel</Trans></ Button>
          ] :
          [
            (warning && <span key="warning" style={{ color: 'red', marginRight: '1em' }}> กรุณาอ่านและทำความเข้าใจทั้งหมดก่อน</ span>),
            (!logined && <Button key="cancel" size={isMobile ? "small" : "middle"} onClick={() => setShowModalConsent(false)}><Trans>consent.btn.cancel</Trans></Button>),
            <Button key="submit" loading={isSubmitting} size={isMobile ? "small" : "middle"} type="primary" onClick={() => onInform()}><Trans>consent.btn.inform</Trans></Button>
          ]
      }
    >
      <Row gutter={12}>
        {
          Object.keys(consent.consent1).map((x) => {
            return (x !== 'header' && x !== 'result' && x !== 'btn') &&
      ...
      }} style={{ color: 'green' }}>ฉันอ่านและทำความเข้าใจแล้ว</Checkbox>
    </Modal>
  );
};
