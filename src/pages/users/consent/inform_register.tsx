...
const Consent: FC<ParentProps> = (props: any) => {
  ...
  const informed = useSelector((state: RootState) => state.user.informConsent?.CovidCenter?.inform?.register);
  const isSubmitting = useSelector((state: RootState) => state.status.submit.set_user_consent.isSubmitting);
  const isMobile = useMediaQuery({ query: '(max-width: 576px)' });
  const { showModalConsent, setShowModalConsent, setShowModalRegister, status } = props;
  ...
  const onInform = () => {
    ...
    if (confirmInform) {
      ...
      if (!logined) { // ตอนลงทะเบียน
        ...
        dispatch(setUserConsentAction.request({ register: true }));
        setLoading(false);
      }
    } else {
      setWarning(true);
  ...
  return (
    <Modal
          ...
          <Col span={12} xs={logined ? 24 : 21} md={logined ? 24 : 21} >
            <Row>
              <Col style={{ fontSize: (isMobile ? "1em" : "1.2em"), fontWeight: 'bold', lineHeight: '1.3em' }}>
                <Trans>consent1.header</Trans>
                <span style={{ color: 'green' }}> <Trans>consent1.for</Trans></span>
              </Col>
            </Row>
            {logined && (
              <Row style={{ marginTop: '10px' }}>
                <Col style={{ fontSize: (isMobile ? ".9em" : "1em"), fontWeight: 'bold', lineHeight: '1.3em', color: (informed ? '#3bb300' : 'red') }}>
                  <Trans>{informed ? "consent1.result.yes" : "consent1.result.no"}</Trans>
                </Col>
              </Row>
            )}
          </Col>
      ...
      footer={
        informed === true ?
          [
            <Button key="cancel" size={isMobile ? "small" : "middle"} onClick={() => setShowModalConsent(false)}> <Trans>consent1.btn.cancel</Trans></ Button>
          ] :
          [
            (warning && <span key="warning" style={{ color: 'red', marginRight: '1em' }}><Trans>consent1.warning</Trans></ span>),
            (!logined && <Button key="cancel" size={isMobile ? "small" : "middle"} onClick={() => setShowModalConsent(false)}><Trans>consent1.btn.cancel</Trans></Button>),
            <Button loading={isSubmitting} key="submit" size={isMobile ? "small" : "middle"} type="primary" onClick={() => onInform()}><Trans>consent1.btn.inform</Trans></Button>
          ]
      }
    >
      <Row gutter={12}>
      ...
      }} style={{ color: 'green' }}><Trans>consent1.btn.checkbox</Trans></Checkbox>
    </Modal>
  );
};
